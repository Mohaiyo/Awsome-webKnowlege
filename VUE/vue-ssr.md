...
通过cdn加速可以把静态资源直接缓存到距离用户最近的节点上，让用户更加快的获取到静态的资源

## TTFB时间过长的原因（超过2s）

经过排查 主要原因是因为接口问题，在服务端渲染，之所以TTFB时间那么长,是因为首页数据请求的接口 响应时间太长导致。需要后台去整改之后看一下效果如何

用户二次请求 是通过serviceWorker 去获取已经加载过的页面，因为我们在vue-ssr中使用到了sw-precache-webpack-plugin插件，首页二次加载之所以时间会到达2秒以上
主要的原因是因为使用了`networkFirst`这一策略。这个时候首页的数据还是优先去服务端接口获取。所以导致如果接口响应时间过长，首屏加载的时间也会特别的长，如果是在离线的状态下。你会发现serviceworker的首屏响应时间只有10几毫秒。所以现在先把锅甩给后端，让后端先去优化接口响应时间过长的问题。才能根本上的解决首屏加载时间过长的问题。ok~
